import { Gift, Video } from "lucide-react-native";
...
type Props = {
  visible: boolean;
  /** Free video compressions the user has left today. */
  remaining: number;
...
/**
 * Free-tier opt-in dialog shown before a rewarded ad unlocks a single video
 * compression. A custom `Modal` (matching `DeleteConfirmationDialog`) that
 * prominently shows how many free video compressions remain today.
 */
export function VideoCompressAdDialog({ visible, remaining, limit, onCancel, onConfirm }: Props) {
      ...
      <Pressable onPress={onCancel} style={{ flex: 1, backgroundColor: "rgba(15,23,42,0.45)", justifyContent: "center", padding: 26 }}>
        <Pressable onPress={() => undefined} style={{ backgroundColor: theme.surface, borderRadius: 22, padding: 24, gap: 16 }}>
          ...
          <Text selectable style={{ color: theme.text, fontSize: 22, fontWeight: "900", textAlign: "center" }}>
            {t("compressionDetail.watchAdTitle")}
          </Text>
          <Text selectable style={{ color: theme.muted, fontSize: 16, lineHeight: 22, textAlign: "center" }}>
            {t("compressionDetail.watchAdBody")}
          </Text>
          {/* Prominent "X of N free today" badge. */}
          <View
            style={{
              alignSelf: "center",
              flexDirection: "row",
          ...
          >
            ...
            <Text style={{ color: theme.text, fontSize: 15, fontWeight: "900" }}>
              {t("compressionDetail.freeVideosLeft", { count: remaining, limit })}
            </Text>
          </View>
